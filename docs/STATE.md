...
| Current gate | G9 — HOLD (FINAL-01/02 fixed; awaiting the final source check) · G10 — HOLD (no APK until that check; this environment cannot build it) |
...
| Last verified | typecheck clean · lint clean · 65 suites / 624 tests · config introspection OK · Android JS bundle exported |
...
| D12 | Correcting an original pack re-derives ALL its opened children (archived included, status unchanged) in the same transaction; a child whose parent cannot be read fails closed (P1-REOPEN-01) | The pack's hard date must always constrain every opened part |
| D13 | Any unsettled restore closes a global write gate; the app shows only the recovery screen until Retry settles the journal AND the strict reload succeeds; only then does the gate open (P1-REOPEN-02, FINAL-02) | Nothing entered during an unsettled restore can later vanish |
